...
export default function LeadTable({ onSelectLead, onOpenUploader, onOpenManualAdd }: LeadTableProps) {
  ...
  const getStatusBadge = (status: LeadStatus) => {
    switch (status) {
      ...
      case 'enriching':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-cyan-50 dark:bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border border-cyan-200 dark:border-cyan-500/20 animate-pulse">
      ...
      case 'drafted':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-sky-50 dark:bg-sky-500/10 text-sky-600 dark:text-sky-400 border border-sky-200 dark:border-sky-500/20">
      ...
      case 'approved':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20">
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 rounded-2xl bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2.5 text-xs">
          ...
          {/* Cards vs Table Switcher Toggle */}
      ...
      {/* Batch Actions Bar (Visible when rows selected in Table Mode) */}
      ...
      {/* Render Card Grid View OR Table Grid View */}
      {viewMode === 'cards' ? (
        <LeadCardGrid
          leads={filteredLeads}
        ...
        <div className="rounded-2xl bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          ...
          <div className="p-4 bg-slate-50 dark:bg-[#0B1120] border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 font-medium">
            ...
            <span>Click any lead card or row to edit AI pitch & review scraped intelligence</span>
          </div>
        </div>
      )}
    </div>
